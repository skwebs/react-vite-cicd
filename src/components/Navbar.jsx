        ...
        <>
            <nav className={`${className} bg-white  py-2.5 dark:bg-gray-900 w-full border-b border-gray-200 dark:border-gray-600`}>
                <ConstantWideLayout className="min-w-[300px] px-2 flex flex-wrap items-center justify-between">
                    ...
                    <div className={`${toggleNav ? 'hidden' : ''} items-center justify-between w-full md:flex md:w-auto md:order-1`} id="navbar-sticky">
                        <ul className="flex flex-col p-4 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                            {links.map((link, i) => (
                                <li key={i}>
                                    <NavLink onClick={() => setToggleNav(true)} to={link.linkUrl} className="block py-2 pl-3 pr-4 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">{link.linkName}</NavLink>
                                </li>
